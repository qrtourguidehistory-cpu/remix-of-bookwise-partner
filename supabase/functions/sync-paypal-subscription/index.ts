...
interface SyncPayPalSubscriptionRequest {
...
serve(async (req) => {
  ...
  try {
    ...
    const body: SyncPayPalSubscriptionRequest = await req.json();
    const { subscription_id } = body;

    if (!subscription_id) {
      return new Response(
        JSON.stringify({ success: false, error: 'Missing required parameter: subscription_id' }),
    ...
    if (!subscriptionData.paypal_subscription_id) {
      return new Response(
        JSON.stringify({ success: false, error: 'PayPal subscription ID not found' }),
        { status: 400, headers: corsHeaders }
      );
    }
    ...
    if (paypalStatus === 'active') {
      dbStatus = 'active';
    } else if (paypalStatus === 'trialing') {
      dbStatus = 'trialing';
    } else if (paypalStatus === 'suspended' || paypalStatus === 'cancelled') {
      dbStatus = paypalStatus === 'suspended' ? 'suspended' : 'cancelled';
    } else if (paypalStatus === 'past_due') {
      dbStatus = 'past_due';
    }
    ...
    console.error('Error in sync-paypal-subscription:', error);
    return new Response(
